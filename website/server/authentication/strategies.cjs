...
function createDiscordStrategy(usersCollection, credsCollection) {
    return new DiscordStrategy({
        ...
        try {
            ...
            if (!credential) {
                if (!profile.email) {
                    return cb(new Error('No email was provided.'));
                }
            ...
            console.error('Error while fetching Discord OAuth:', error);
            return cb(error);
        }
    });
}

module.exports = { createGoogleStrategy, createDiscordStrategy };